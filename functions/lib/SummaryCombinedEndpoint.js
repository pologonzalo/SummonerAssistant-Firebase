...
module.exports = class SummaryCombinedEndpoint extends Endpoint {
	...
	Parse($, json) {
		...
		data.summonerId = Boolean(regex) && regex.length ? parseInt(regex[1]) : undefined
		...
		if (temp && temp.length > 0 && temp[2]) {
			...
			$('.GameItemWrap').each((i, game) => {
				...
				game.id = Boolean(regex) && regex.length ? parseInt(regex[1]) : undefined
				...
				$('.Items .ItemList .Item').each((j, _item) => {
					...
					item.id = Boolean(regex) && regex.length ? parseInt(regex[1]) : undefined
					items.push(item)
				});
				game.items = items
				...
				trinket.id = Boolean(regex) && regex.length ? parseInt(regex[1]) : undefined
